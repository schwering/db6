...
package body Timers is
   ...
   pragma Import (C, C_clock, "timer_clock");

   function C_time return Interfaces.C.unsigned_long;
   pragma Import (C, C_time, "timer_time");

   C_CLOCKS_PER_SEC : constant Interfaces.C.unsigned_long;
   pragma Import (C, C_CLOCKS_PER_SEC, "timer_CLOCKS_PER_SEC");

   C_TIMES_PER_SEC : constant Interfaces.C.unsigned_long;
   pragma Import (C, C_TIMES_PER_SEC, "timer_TIMES_PER_SEC");
   ...
   procedure Print_Relative
     (S : in String;
      T : in Timer_Type;
      C : in Long_Long_Integer)
   is
      Real_Millis : constant Long_Long_Integer :=
         Long_Long_Integer(Real_Time(T));
      Relative_Picos : constant Long_Long_Integer :=
         Real_Millis * 10**6 / C;
      Tmp : constant String := Relative_Picos'Img;
      P1 : constant String := Tmp(Tmp'First .. Tmp'Last - 3);
      P2 : constant String := Tmp(Tmp'Last - 2 .. Tmp'Last);
      N  : constant String := P1 &"."& P2;
   begin
      Ada.Text_IO.Put_Line(S &" takes"& N &" ns");
   end Print_Relative;
